...
func main() {
	...
	// Keep process alive after initialization; watcher.Start() returns once InitRun succeeds.
	...
	for {
		...
		case <-sigChan:
			...
			if err := watcher.Stop(); err != nil {
				log.Printf("[BOOT] stop error: %v", err)
			}
			return
		case <-stateTick.C:
			switch watcher.GetState() {
			case hersh.StateStopped, hersh.StateKilled, hersh.StateCrashed:
				log.Printf("[BOOT] watcher terminated state=%s", watcher.GetState())
				return
			}
		}
	}
