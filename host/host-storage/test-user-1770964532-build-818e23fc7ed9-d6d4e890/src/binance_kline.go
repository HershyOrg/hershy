...
func (b *BinanceKlineStream) Connect() error {
	...
	for attempt := 1; attempt <= 10; attempt++ {
		...
		if b.stopped.Load() {
			return fmt.Errorf("stream stopped while connecting")
		}
		fmt.Printf("[WARN] connect attempt %d/10 failed: %v\n", attempt, err)
		time.Sleep(2 * time.Second)
	}
	if err != nil {
		return fmt.Errorf("failed to connect to Binance after retries: %w", err)
	}
	b.mu.Lock()
	b.wsConn = conn
	b.mu.Unlock()
