...
} from "@/mock-data/client/binance-demo-api";
...
function NodeEditorInner({ initialGraph, initialGraphVersion = 0, programCode = "", previewMode = false }: NodeEditorProps) {
  ...
  const latestProgramCodeRef = useRef(programCode);
  ...
  useEffect(() => {
    latestProgramCodeRef.current = programCode;
  }, [programCode]);
  ...
  useEffect(() => {
    ...
    setNodes((currentNodes) => {
      ...
      return sanitizeEditorGraph(enriched).nodes;
    });
