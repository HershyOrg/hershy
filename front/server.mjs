...
import { loadHershyLibraryContext } from './server/hershyContext.mjs';
import {
  fetchMarketChartSeries,
  fetchMarketOverviewRows,
} from './server/marketData.mjs';
import {
  buildPreviewValues,
  derivePayloadFields,
  normalizeStreamSampleInput,
  parseResponseSchemaFields,
  sampleStreamPayload,
} from './server/streamSampler.mjs';
import {
  ALLOWED_RAW_ACTION_INPUT_REASONS,
  CHAIN_ALIASES,
  DEFAULT_MARKET_OVERVIEW_ROWS,
  ETHERSCAN_V2_ENDPOINT,
  EXPLORER_API_ENDPOINTS,
  EXPLORER_CHAIN_IDS,
  EXPLORER_WEB_BASE_URLS,
  GENERIC_SIGNAL_TOKENS,
  HOP_BY_HOP_HEADERS,
  ORCHESTRATION_PLAN_SCHEMA,
  RAW_FEED_SIGNAL_TOKENS,
  REQUIRED_SIGNAL_BY_STRATEGY_KIND,
  RESEARCH_BUNDLE_SCHEMA,
  STRATEGY_BLOCK_TYPE_ALIASES,
  STRATEGY_CONNECTION_KIND_ALIASES,
  STRATEGY_GRAPH_SCHEMA,
  STRATEGY_OVERVIEW_SCHEMA,
  STRATEGY_RUNNER_RUNTIME_FILES,
  TIME_STRATEGY_KIND_SET,
} from './server/dataStructures.mjs';
...
const MONACO_WORKER_ALIASES = new Map([
  ...
  ['/monaco/vs/language/json/json.worker.js', 'json.worker-'],
  ['/monaco/vs/language/css/css.worker.js', 'css.worker-'],
  ['/monaco/vs/language/html/html.worker.js', 'html.worker-'],
  ['/monaco/vs/language/typescript/ts.worker.js', 'ts.worker-'],
  ['/monaco/vs/language/typescript/typescript.worker.js', 'ts.worker-'],
]);

function resolveMonacoWorkerAsset(prefix) {
  try {
    const fileName = fsSync
      .readdirSync(MONACO_ASSETS_DIR)
      .find((entry) => entry.startsWith(prefix) && entry.endsWith('.js'));

    return fileName ? path.join(MONACO_ASSETS_DIR, fileName) : null;
  } catch {
    return null;
  }
}
...
for (const [route, assetPrefix] of MONACO_WORKER_ALIASES.entries()) {
  app.get(route, (req, res) => {
    void req;
    const workerPath = resolveMonacoWorkerAsset(assetPrefix);
    if (!workerPath) {
      res.sendStatus(404);
      return;
    }
...
app.use('/monaco/vs', express.static(MONACO_VS_DIR, {
...
async function buildAIStrategyUserPrompt(prompt, currentStrategy, researchBundle, orchestrationPlan, exchangeConnections = [], userContext = null) {
  ...
  const hershyContext = await loadHershyLibraryContext(prompt, { repoRoot: REPO_ROOT });
  if (hershyContext) {
...
- runtimeGraph.metadata.workflowGroups must contain only visible trading/runtime sequences. Never repair by adding AI-loop phases such as intent parsing, RAG, context retrieval, web/research discovery, KG retrieval, candidate building, ranking/scoring research, solver planning, or workflow planning.
...
const RUNTIME_GRAPH_INTERNAL_WORKFLOW_RE = /\b(intent|research|retrieval|rag|retrieval[-\s]*augmented|knowledge\s*retrieval|context\s*retrieval|knowledge\s*graph|kg|web\s*discovery|candidate\s*universe|pool\s*discovery|implementation\s*research|orchestration|planner|planning|ranking|ranker|solver|evidence|adapter|labeling|workflow\s*plan)\b|의도|리서치|검색|후보|지식\s*검색|지식\s*그래프|랭킹|순위|계획|근거|증거|어댑터|라벨|오케스트레이션/i;

function workflowGroupLooksLikeInternalPreparation(group) {
  return RUNTIME_GRAPH_INTERNAL_WORKFLOW_RE.test(`${group.id} ${group.title} ${group.purpose}`);
}
...
function lintRuntimeStrategyGraph(strategyGraph, { prompt = '', intentPlan = null, logicIR = null, exchangeConnections = [] } = {}) {
  ...
  if (workflowGroups.length === 0) {
    ...
    for (const group of workflowGroups) {
      if (workflowGroupLooksLikeInternalPreparation(group)) {
        issues.push(logicIssue(
          'WORKFLOW_GROUP_INTERNAL_PREPARATION',
          'error',
          `Workflow ${group.id} looks like RAG/research/planning, not visible runtime strategy.`,
          { workflowId: group.id, title: group.title, purpose: group.purpose },
          'Move RAG/KG retrieval/research/planning evidence into intentPlan, logicIR assumptions, orchestration notes, or evidence context. runtimeGraph.workflowGroups should only contain visible trading/runtime sequences.',
        ));
      }
