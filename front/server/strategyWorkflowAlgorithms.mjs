...
const EXECUTION_DOMAINS = {
  general_automation: {
    ...
    title: 'General Strategy Workflow',
...
const STRATEGY_ALGORITHMS = {
  generic_strategy_workflow: {
    id: 'generic_strategy_workflow',
    title: 'Generic Strategy Workflow',
...
function chooseAlgorithm(prompt) {
  ...
  return {
    ...STRATEGY_ALGORITHMS.generic_strategy_workflow,
    confidence: 0.45,
    reason: 'No specialized workflow matched; using generic strategy workflow generation.',
  };
}
...
function detectExecutionDomain(prompt, algorithm, options = {}) {
  ...
  if (algorithm.id === 'generic_strategy_workflow' && !mentionsCEX && !mentionsOnchain) {
...
function buildWorkflowForAlgorithm(algorithm, prompt, executionDomain) {
  if (algorithm.id === 'generic_strategy_workflow') {
...
export function buildStrategyWorkflowPromptSection(workflowPlan) {
  ...
  return [
    ...
    'Hard rule: RAG/KG retrieval, context discovery, ranking, and planning are evidence-gathering steps outside the visible runtime graph; do not emit them as runtimeGraph blocks or workflowGroups.',
  ].join('\n');
}
