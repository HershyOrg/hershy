...
import type { BlockData, ChartComparisonValue, FunctionNodeData, IndicatorCondition, NodeChartPoint } from "../types/editorTypes";
import {
...
type VisualizationFormat = "chart" | "ladder" | "log";
...
function getBlockVisualizationFormat(block: IndicatorOutputBlock): VisualizationFormat {
  return normalizeVisualizationFormat(block.visualizationFormat) ??
    normalizeVisualizationFormat(block.visualFormat) ??
    normalizeVisualizationFormat(block.visualType) ??
    normalizeVisualizationFormat(block.chartType) ??
    "chart";
}
...
function getBlockLadderSide(block: IndicatorOutputBlock): "upper" | "lower" | null {
  return normalizeLadderSide(block.ladderSide) ??
    normalizeLadderSide(block.bookSide) ??
    normalizeLadderSide(block.side) ??
    normalizeLadderSide(block.levelSide) ??
    normalizeLadderSide(block.name) ??
    normalizeLadderSide(block.id);
}
...
type ReturnOutputInfo = ReturnObjectInfo & {
  outputSpecs: ReturnOutputSpec[];
};
...
function sanitizeReturnOutputName(value: string, fallback = "data") {
  return value
    .trim()
    .replace(/^['"]|['"]$/g, "")
    .replace(/^[{[(]\s*|\s*[}\])]$/g, "")
...
function getTopLevelColonIndex(source: string) {
  let depth = 0;
...
function stripReturnValueWrapper(source: string) {
...
function parseOutputSpecsFromList(source: string, visualizationFormat?: VisualizationFormat): ReturnOutputSpec[] {
  const content = stripReturnValueWrapper(source);
  return splitTopLevelObjectProperties(content)
    .map((property, index): ReturnOutputSpec | null => {
      const colonIndex = getTopLevelColonIndex(property.text);
      const keySource = colonIndex >= 0 ? property.text.slice(0, colonIndex) : property.text;
      const key = getObjectPropertyKey(keySource) ||
        sanitizeReturnOutputName(keySource.match(/[A-Za-z_$][\w$]*(?:\.[A-Za-z_$][\w$]*)*/)?.[0] ?? "", `data${index + 1}`);
      return key ? { key, visualizationFormat } : null;
    })
    .filter((item): item is ReturnOutputSpec => item !== null);
}

function getReturnOutputSpecSignature(outputSpecs: ReturnOutputSpec[]) {
  return outputSpecs
    .map((spec) => `${spec.key}:${spec.visualizationFormat ?? "chart"}`)
    .join("|");
}

function getOutputBlockSpecSignature(outputBlocks: IndicatorOutputBlock[]) {
  return outputBlocks
    .map((block) => `${block.name}:${getBlockVisualizationFormat(block)}`)
    .join("|");
}

function extractBracketLogOutputInfo(code: string): ReturnOutputInfo | null {
  const match = code.match(/\[\s*log\s*:\s*([\s\S]*?)\]/i);
  if (!match?.[1]) return null;

  const outputSpecs = parseOutputSpecsFromList(match[1], "log");
  if (outputSpecs.length === 0) return null;

  return {
    objectStart: match.index ?? 0,
    objectEnd: (match.index ?? 0) + match[0].length - 1,
    keys: outputSpecs.map((spec) => spec.key),
    properties: outputSpecs.map((spec, index) => ({
      key: spec.key,
      start: index,
      end: index,
      text: spec.key,
...
function extractReturnedOutputInfo(code: string): ReturnOutputInfo | null {
  const bracketLogInfo = extractBracketLogOutputInfo(code);
  if (bracketLogInfo) return bracketLogInfo;

  const objectInfo = extractReturnedObjectInfo(code);
  if (!objectInfo) return null;

  const outputSpecs = objectInfo.properties.flatMap((property) => {
    if (property.key.toLowerCase() !== "log") {
      return [{ key: property.key }];
    }

    const colonIndex = getTopLevelColonIndex(property.text);
    const valueText = colonIndex >= 0 ? property.text.slice(colonIndex + 1) : property.text;
    const logSpecs = parseOutputSpecsFromList(valueText, "log");
    return logSpecs.length > 0 ? logSpecs : [{ key: "log", visualizationFormat: "log" as const }];
  });
...
function removeReturnOutputProperty(code: string, propertyName: string) {
  const logMatch = code.match(/\[\s*log\s*:\s*([\s\S]*?)\]/i);
  if (logMatch?.[1] && logMatch.index !== undefined) {
    const specs = parseOutputSpecsFromList(logMatch[1], "log");
    const remaining = specs.filter((spec) => spec.key !== propertyName);
    if (remaining.length !== specs.length) {
      const replacement = remaining.length > 0
        ? `[log: ${remaining.map((spec) => spec.key).join(", ")}]`
        : "";
      return `${code.slice(0, logMatch.index)}${replacement}${code.slice(logMatch.index + logMatch[0].length)}`;
    }
  }

  return removeReturnObjectProperty(code, propertyName);
}
...
function createOutputBlockClone(source: IndicatorOutputBlock, index: number, existingBlocks: IndicatorOutputBlock[]): IndicatorOutputBlock {
...
function readLadderValue(value: unknown): number | null {
  ...
  for (const key of ["value", "amount", "size", "qty", "quantity", "depth", "liquidity", "price", "last"]) {
    const parsed = readLadderValue(record[key]);
    if (parsed !== null) return parsed;
  }
  return null;
}

function readLadderLabel(value: unknown, fallback: string) {
  if (!value || typeof value !== "object") return fallback;
  const record = value as Record<string, unknown>;
  for (const key of ["label", "name", "id", "key", "price"]) {
...
function readBlockLadderValues(block: IndicatorOutputBlock) {
  const raw = block.ladderRows ?? block.ladderValues ?? block.rows ?? block.values;
  if (!Array.isArray(raw)) return [];
  return raw
    .map((item, index) => {
      const value = readLadderValue(item);
      if (value === null) return null;
      return {
        id: `${block.id}-row-${index}`,
        label: readLadderLabel(item, `${block.name} ${index + 1}`),
        value,
      };
    })
...
function normalizeLadderRows(rows: Array<Omit<LadderRow, "normalized">>): LadderRow[] {
  const values = rows.map((row) => row.value).filter((value) => Number.isFinite(value));
  if (values.length === 0) {
    return rows.map((row) => ({ ...row, normalized: 0.62 }));
  }
  ...
  return rows.map((row) => {
    ...
    if (Number.isFinite(span) && span > Number.EPSILON) {
      normalized = (row.value - min) / span;
    } else if (maxAbs > Number.EPSILON) {
      normalized = Math.abs(row.value) / maxAbs;
    }

    return {
      ...row,
      normalized: clampDimension(normalized, 0.08, 1),
    };
  });
}

function buildLadderRows(models: OutputChartModel[]): LadderRow[] {
  const explicitSides = models.map((model) => model.ladderSide);
  const hasExplicitSide = explicitSides.some(Boolean);
  const splitIndex = Math.ceil(models.length / 2);
  const rawRows: Array<Omit<LadderRow, "normalized">> = [];

  models.forEach((model, modelIndex) => {
    const fallbackSide = modelIndex < splitIndex ? "upper" : "lower";
    const side = model.ladderSide ?? (hasExplicitSide ? "lower" : fallbackSide);
    const nestedRows = readBlockLadderValues(model.block);
    ...
    rawRows.push({
      id: model.block.id,
      label: model.block.name || `row ${modelIndex + 1}`,
      value: model.latestValue,
      previousValue: model.series.at(-2)?.value,
      side,
      sourceModel: model,
    });
  });

  return normalizeLadderRows(rawRows);
}

function getGroupVisualizationFormat(group: OutputChartGroup): VisualizationFormat {
  if (group.models.some((model) => model.visualizationFormat === "log")) return "log";
  if (group.models.some((model) => model.visualizationFormat === "ladder")) return "ladder";
  if (group.models.some((model) => model.ladderSide)) return "ladder";
  return "chart";
}
...
function NormalizedLadderView({
  ...
  const upperRows = rows.filter((row) => row.side === "upper");
  const lowerRows = rows.filter((row) => row.side === "lower");
  const hasRows = rows.length > 0;
  const fallbackSplitIndex = Math.ceil(rows.length / 2);
  const displayUpperRows = upperRows.length ? upperRows : rows.slice(0, fallbackSplitIndex);
  const displayLowerRows = lowerRows.length ? lowerRows : upperRows.length ? [] : rows.slice(fallbackSplitIndex);

  const renderRow = (row: LadderRow) => {
    const width = `${Math.round(row.normalized * 100)}%`;
    const trend = Number.isFinite(row.previousValue)
      ? row.value - Number(row.previousValue)
      : 0;
    const isUpper = row.side === "upper";

    return (
      <div
        key={row.id}
        className={cn(
          "relative grid h-7 grid-cols-[minmax(0,1fr)_86px_42px] items-center overflow-hidden border-b border-white/5 px-2 text-xs",
          isUpper ? "bg-rose-500/[0.035]" : "bg-emerald-500/[0.035]",
        )}
      >
        <div
          className={cn(
            "absolute inset-y-0 opacity-55 transition-[width] duration-300",
            isUpper ? "right-0 bg-rose-500/30" : "left-0 bg-emerald-500/30",
          )}
          style={{ width }}
        />
        <div className="relative z-10 min-w-0 truncate font-black text-slate-100">
          {row.label}
        </div>
        <div className="relative z-10 text-right font-mono font-black text-slate-50">
          {formatValue(row.value)}
        </div>
        <div
          className={cn(
            "relative z-10 text-right font-mono text-[10px] font-black",
            trend > 0 ? "text-emerald-300" : trend < 0 ? "text-rose-300" : "text-slate-500",
          )}
        >
          {Math.round(row.normalized * 100)}
        </div>
      </div>
    );
  };
  ...
  return (
    <div className={cn("flex h-full min-h-0 flex-col bg-slate-950", isFocused && "ring-1 ring-inset ring-[#f0b90b]")}>
      <div className="flex h-8 shrink-0 items-center justify-between gap-3 border-b border-slate-800 bg-slate-950 px-2">
        <div className="flex min-w-0 items-center gap-2">
          <span className="truncate rounded bg-[#f0b90b]/20 px-2 py-1 text-[10px] font-black uppercase tracking-[0.12em] text-[#fcd535]">
            Ladder
          </span>
          <span className="truncate text-[11px] font-black text-slate-200">{label}</span>
          <span className="rounded bg-slate-800 px-1.5 py-0.5 text-[9px] font-black text-slate-400">
            normalized rows
          </span>
        </div>
        {!compact ? (
      ...
      <div className="grid h-5 shrink-0 grid-cols-[1fr_auto_1fr] items-center border-b border-slate-800 px-2 text-[9px] font-black uppercase tracking-[0.14em] text-slate-500">
        <span>upper {upperRows.length}</span>
        <span className="rounded bg-slate-900 px-2 py-0.5 text-slate-400">mid</span>
        <span className="text-right">lower {lowerRows.length}</span>
      </div>
      <div className="min-h-0 flex-1 overflow-hidden">
        {hasRows ? (
          <div className="flex h-full min-h-0 flex-col">
            <div className="min-h-0 flex-1 overflow-auto border-b border-slate-800">
              {displayUpperRows.map(renderRow)}
            </div>
            <div className="min-h-0 flex-1 overflow-auto">
              {displayLowerRows.map(renderRow)}
            </div>
          </div>
        ) : (
          <div className="flex h-full items-center justify-center text-xs font-semibold text-slate-500">
            Ladder rows unavailable
          </div>
        )}
      </div>
    </div>
...
function LogOutputView({
  ...
  const rows = models.map((model) => {
    const latestPoint = model.series.at(-1);
    const timeValue = typeof latestPoint?.time === "number" && Number.isFinite(latestPoint.time)
      ? latestPoint.time
      : Date.now();
    return {
      id: model.block.id,
      name: model.block.name,
      value: formatOutputValue(model.latestValue, model.isBooleanChart),
      rawValue: model.latestValue,
      time: new Date(timeValue).toLocaleTimeString("ko-KR", { hour12: false }),
      source: model.chartSource || model.functionCodeGroupLabel || "indicator",
    };
  });
  const objectPreview = rows
    .map((row) => `${row.name}: ${row.value}`)
    .join(", ");

  return (
    <div className={cn("flex h-full min-h-0 flex-col bg-[#0b0e11]", isFocused && "ring-1 ring-inset ring-[#f0b90b]")}>
      <div className="flex h-8 shrink-0 items-center justify-between gap-3 border-b border-slate-800 bg-slate-950 px-2">
        <div className="flex min-w-0 items-center gap-2">
          <span className="flex items-center gap-1 rounded bg-emerald-400/15 px-2 py-1 text-[10px] font-black uppercase tracking-[0.12em] text-emerald-300">
      ...
      <div className="min-h-0 flex-1 overflow-auto p-2 font-mono text-[11px] leading-5 text-slate-300">
        <div className="mb-2 rounded border border-slate-800 bg-slate-950/80 px-2 py-1 text-emerald-200">
          <span className="text-slate-500">log </span>
          {"{ "}
          <span className="text-slate-100">{objectPreview || "empty"}</span>
          {" }"}
        </div>
        {rows.map((row) => (
          <div
            key={row.id}
            className="grid grid-cols-[68px_minmax(0,1fr)_auto] items-center gap-2 border-b border-slate-800/70 px-1 py-1 last:border-b-0"
            title={row.source}
          >
            <span className="text-slate-500">{row.time}</span>
            <span className="min-w-0 truncate text-slate-200">{row.name}</span>
            <span className={cn(
              "rounded bg-slate-900 px-1.5 py-0.5 text-right font-black",
              row.rawValue >= 0 ? "text-emerald-300" : "text-rose-300",
            )}>
              {row.value}
            </span>
          </div>
...
function FunctionNodeComponent({
  ...
  const [areOutputBlocksExpanded, setAreOutputBlocksExpanded] = useState(Boolean(data.outputBlocksExpanded));
    ...
    () => {
      const models = chartableOutputBlocks.map((block, index) => {
        ...
        return {
          block,
          index,
          visualizationFormat: getBlockVisualizationFormat(block),
          ladderSide: getBlockLadderSide(block),
          isBooleanChart,
  ...
  const primaryOutputGroup = outputChartGroups[0] ?? null;
  const primaryGroupVisualizationFormat = primaryOutputGroup ? getGroupVisualizationFormat(primaryOutputGroup) : "chart";
  const primaryLadderRows = useMemo(
    () => primaryGroupVisualizationFormat === "ladder" && primaryOutputGroup
      ? buildLadderRows(primaryOutputGroup.models)
      : [],
    [primaryGroupVisualizationFormat, primaryOutputGroup],
  );
  ...
  const triggerVisibleOutputBlocks = useMemo(
    () => visibleOutputBlocks.filter(isTriggerDataBlock),
    [visibleOutputBlocks],
  );
  const outputBlocksAreCollapsible = normalVisibleOutputBlocks.length > 1;
  const outputBlocksAreCollapsed = outputBlocksAreCollapsible && !areOutputBlocksExpanded;
  const listedOutputBlocks = useMemo(() => {
    if (!outputBlocksAreCollapsed) return visibleOutputBlocks;

    const selected = new Map<string, BlockData>();
    const firstNormalBlock = normalVisibleOutputBlocks[0];
    if (firstNormalBlock) selected.set(firstNormalBlock.id, firstNormalBlock);
    normalVisibleOutputBlocks.forEach((block) => {
      if (connectedOutputHandleIds.has(getOutputHandleId(id, block.id))) {
        selected.set(block.id, block);
      }
    });
    triggerVisibleOutputBlocks.forEach((block) => selected.set(block.id, block));
    return Array.from(selected.values());
  }, [
    connectedOutputHandleIds,
    id,
    normalVisibleOutputBlocks,
    outputBlocksAreCollapsed,
    triggerVisibleOutputBlocks,
    visibleOutputBlocks,
  ]);
  const collapsedOutputSummaryCount = Math.max(normalVisibleOutputBlocks.length - 1, 0);
  ...
  useEffect(() => {
    setAreOutputBlocksExpanded(Boolean(data.outputBlocksExpanded));
  }, [data.outputBlocksExpanded]);
  ...
  useEffect(() => {
    const nodeCode = typeof data.code === "string" ? data.code : "";
    if (!nodeCode.trim()) return;

    const returnInfo = extractReturnedOutputInfo(nodeCode);
    const hasLogOutput = returnInfo?.outputSpecs.some((spec) => spec.visualizationFormat === "log");
    if (!returnInfo || !hasLogOutput) return;

    const expectedSignature = getReturnOutputSpecSignature(returnInfo.outputSpecs);
    const currentSignature = getOutputBlockSpecSignature(outputBlocks);
    const allBlocksUseCurrentCode = outputBlocks.every((block) => getOutputFormulaCode(block, "") === nodeCode);
    if (currentSignature === expectedSignature && allBlocksUseCurrentCode) return;

    const blocksByName = new Map(outputBlocks.map((block) => [block.name, block]));
    const firstBlock = outputBlocks[0];
    const nodeLabel = readStringProperty(data, ["functionName", "label"]) || "function";
    const groupId = readStringProperty(firstBlock, [
      "functionCodeGroupId",
      "functionGroupId",
      "codeGroupId",
      "sourceFunctionId",
    ]) || `fn-${sanitizeGroupId(nodeLabel || id)}-${hashStableString(nodeCode).slice(0, 6)}`;
    const groupLabel = readStringProperty(firstBlock, ["functionCodeName", "functionName", "codeGroupName"]) || nodeLabel;

    const nextOutputBlocks = returnInfo.outputSpecs.map((spec, index) => {
      const existing = blocksByName.get(spec.key) ?? outputBlocks[index];
      return {
        ...(existing ?? {}),
        ...
        functionCodeGroupId: groupId,
        functionCodeName: groupLabel,
        visualizationFormat: spec.visualizationFormat ??
          (existing?.visualizationFormat === "log" ? "chart" : existing?.visualizationFormat) ??
          "chart",
      };
    });

    const nextOutputIds = new Set(nextOutputBlocks.map((block) => block.id));
    const nextSelectedOutputId = nextOutputBlocks[0]?.id ?? null;
    if (selectedOutputId && !nextOutputIds.has(selectedOutputId)) {
      setSelectedOutputId(nextSelectedOutputId);
    }
    if (codeOutputId && !nextOutputIds.has(codeOutputId)) {
      setCodeOutputId(nextSelectedOutputId);
    }
    if (returnInfo.outputSpecs.length > 1) {
      setAreOutputBlocksExpanded(false);
    }

    updateNodeData({
      outputBlocks: nextOutputBlocks,
      ...(returnInfo.outputSpecs.length > 1 ? { outputBlocksExpanded: false } : {}),
    } as Partial<FunctionNodeData>);
  }, [codeOutputId, data, id, outputBlocks, selectedOutputId, updateNodeData]);

  const handleToggleOutputBlocksExpanded = useCallback(() => {
    const nextExpanded = !areOutputBlocksExpanded;
    setAreOutputBlocksExpanded(nextExpanded);
    updateNodeData({ outputBlocksExpanded: nextExpanded });
  }, [areOutputBlocksExpanded, updateNodeData]);
  ...
  const handleGroupVisualizationFormatChange = useCallback(
    (group: OutputChartGroup, visualizationFormat: VisualizationFormat) => {
      const groupBlockIds = new Set(group.models.map((model) => model.block.id));
      updateNodeData({
        outputBlocks: outputBlocks.map((block) =>
          groupBlockIds.has(block.id)
            ? { ...block, visualizationFormat, type: "output" as const }
            : block,
        ),
      } as Partial<FunctionNodeData>);
    },
  ...
  const handleFunctionGroupCodeChange = useCallback(
    (group: OutputChartGroup, value: string | undefined) => {
      ...
      const returnInfo = extractReturnedOutputInfo(nextCode);
      const removedBlockIds = new Set<string>();
      let nextOutputBlocks: IndicatorOutputBlock[];

      if (returnInfo && returnInfo.outputSpecs.length > 0) {
        const firstGroupIndex = Math.max(outputBlocks.findIndex((block) => groupBlockIds.has(block.id)), 0);
        const beforeBlocks = outputBlocks.slice(0, firstGroupIndex).filter((block) => !groupBlockIds.has(block.id));
        const afterBlocks = outputBlocks.slice(firstGroupIndex).filter((block) => !groupBlockIds.has(block.id));
        const groupBlocksByName = new Map(
          outputBlocks
            .filter((block) => groupBlockIds.has(block.id))
            .map((block) => [block.name, block]),
        );

        const nextGroupBlocks = returnInfo.outputSpecs.map((spec, index) => {
          const existing = groupBlocksByName.get(spec.key) ??
            (index === 0 ? outputBlocks.find((block) => groupBlockIds.has(block.id)) : undefined);
          const blockId = existing?.id ?? `ob-${Date.now()}-${index}-${sanitizeGroupId(spec.key)}`;
          return {
            ...(existing ?? {}),
            id: blockId,
            name: spec.key,
        ...
        outputBlocks.forEach((block) => {
          if (groupBlockIds.has(block.id) && !nextGroupBlocks.some((nextBlock) => nextBlock.id === block.id)) {
            removedBlockIds.add(block.id);
          }
        });
        nextOutputBlocks = [...beforeBlocks, ...nextGroupBlocks, ...afterBlocks];
        const shouldAutoCollapseOutputs = returnInfo.outputSpecs.length > 1 && group.models.length <= 1;
        if (shouldAutoCollapseOutputs) {
          setAreOutputBlocksExpanded(false);
        }
      } else {
        nextOutputBlocks = outputBlocks.map((block) => {
      ...
      updateNodeData({
        outputBlocks: nextOutputBlocks,
        ...(returnInfo && returnInfo.outputSpecs.length > 1 && group.models.length <= 1 ? { outputBlocksExpanded: false } : {}),
  ...
  const handleBlockChange = useCallback(
    (blockType: "input" | "output", blockId: string, patch: Partial<BlockData>) => {
      if (blockType === "output") {
        ...
        const { visualizationFormat, ladderSide, ...restPatch } = patch;
        const nextPatch: Partial<IndicatorOutputBlock> = { ...restPatch, type: "output" };
        const normalizedVisualizationFormat = normalizeVisualizationFormat(visualizationFormat);
        const normalizedLadderSide = normalizeLadderSide(ladderSide);
        if (normalizedVisualizationFormat) {
          nextPatch.visualizationFormat = normalizedVisualizationFormat;
        }
        if (normalizedLadderSide) {
          nextPatch.ladderSide = normalizedLadderSide;
        }
        if (
          currentBlock &&
  ...
  const handleAddBlock = useCallback(
    (blockType: "input" | "output") => {
      ...
      if (blockType === "output") {
        ...
        const nextBlock = createOutputBlockClone(cloneSource as IndicatorOutputBlock, outputBlocks.length, outputBlocks);
        setSelectedOutputId(nextBlock.id);
        setAreOutputBlocksExpanded(true);
        updateNodeData({
          outputBlocks: [...outputBlocks, nextBlock],
          outputBlocksExpanded: true,
        } as Partial<FunctionNodeData>);
        return;
      }
      ...
      updateNodeData({
        [key]: [
          ...currentBlocks,
          {
            id: createUniqueBlockId(blockType === "input" ? "ib" : "ob", currentBlocks),
  ...
  const removeOutputBlock = useCallback(
    (blockId: string) => {
      ...
      const currentCode = group
        ...
        : typeof data.code === "string" ? data.code : "";
      const nextCode = currentCode ? removeReturnOutputProperty(currentCode, targetBlock.name) : currentCode;
      const groupBlockIds = new Set(group?.models.map((model) => model.block.id) ?? []);
  ...
  const handleRequestRemoveOutputBlock = useCallback(
    (blockId: string) => {
      ...
      const returnCount = extractReturnedOutputInfo(groupCode)?.keys.length ?? group.models.length;
      if (returnCount > 1) {
      ...
      >
        ...
        <div className="border-b border-slate-100 px-3 py-2">
          ...
          <div className="h-[132px] overflow-hidden rounded-md border border-slate-100 dark:border-slate-700">
            {primaryGroupVisualizationFormat === "log" && primaryOutputGroup ? (
              <LogOutputView
                models={primaryOutputGroup.models}
                label={primaryOutputGroup.label}
                compact
              />
            ) : primaryGroupVisualizationFormat === "ladder" && primaryOutputGroup ? (
              <NormalizedLadderView
                rows={primaryLadderRows}
                label={primaryOutputGroup.label}
                compact
              />
            ) : (
      ...
      <div className="min-w-0">
          ...
          >
            {outputChartGroups.map((group, groupIndex) => {
              ...
              const groupVisualizationFormat = getGroupVisualizationFormat(group);
              const ladderRows = groupVisualizationFormat === "ladder" ? buildLadderRows(group.models) : [];
              ...
              return (
                <Fragment key={group.key}>
                  ...
                  >
                    {hasBracket ? <FunctionCodeBracket /> : null}
                    {isCodeGroup ? (
                    ...
                    ) : groupVisualizationFormat === "log" ? (
                      <div
                        role="button"
                        tabIndex={0}
                        className="min-h-0 flex-1 overflow-hidden outline-none"
                        onClick={() => setSelectedOutputId(group.models[0]?.block.id ?? null)}
                        onFocus={() => setSelectedOutputId(group.models[0]?.block.id ?? null)}
                        onKeyDown={(event) => {
                          if (event.key === "Enter" || event.key === " ") {
                            event.preventDefault();
                            setSelectedOutputId(group.models[0]?.block.id ?? null);
                          }
                        }}
                      >
                        <LogOutputView
                          models={group.models}
                          label={group.label}
                          isFocused={group.models.some((model) => selectedOutput?.block.id === model.block.id)}
                          onSwitchToChart={() => handleGroupVisualizationFormatChange(group, "chart")}
                          onShowCode={() => {
                            const firstModel = group.models[0];
                            if (!firstModel) return;
                            setSelectedOutputId(firstModel.block.id);
                            setCodeOutputId(firstModel.block.id);
                            setViewMode("code");
                            updateNodeData({ viewMode: "code" });
                          }}
                        />
                      </div>
                    ) : groupVisualizationFormat === "ladder" ? (
                      <div
                        role="button"
                        tabIndex={0}
                        className="min-h-0 flex-1 overflow-hidden outline-none"
                      ...
                      return (
                        ...
                        <MetricChart
                          ...
                          rangeKey={`indicator:${id}:${model.block.id}`}
                        />
                              </>
                            </div>
                          </div>
          ...
          <div className="grid grid-cols-2 border-b border-slate-200">
            ...
            <div className="p-3">
              ...
              <div className="space-y-1.5">
                {listedOutputBlocks.map((block) => {
                  const outputBlock = block as IndicatorOutputBlock;
                  const isTriggerOutput = isTriggerDataBlock(block);
                  const isCollapsedSummaryBlock = outputBlocksAreCollapsed && block.id === normalVisibleOutputBlocks[0]?.id;
                        ...
                        <>
                          <div className="flex items-center gap-1 pr-10">
                            <input
                              ...
                              className="min-w-0 flex-1 bg-transparent text-xs font-semibold text-emerald-900 outline-none"
                              placeholder="output name"
                            />
                            {isCollapsedSummaryBlock ? (
                              <button
                                type="button"
                                onClick={(event) => {
                                  event.stopPropagation();
