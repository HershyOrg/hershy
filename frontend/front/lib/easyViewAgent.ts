...
export const DEFAULT_STRATEGY_TEMPLATES: StrategyTemplate[] = [
  {
    ...
    prompt: "XRP 현물과 선물 가격차가 0.5% 이상 벌어지면 현물을 매수하고 선물을 숏으로 헤지해줘",
    tags: ["XRP", "Hedge", "1m"],
  },
  {
    id: "trend",
...
export function buildStrategyCodeFromTemplate(template: StrategyTemplate): string {
  ...
  return `strategy "XRP spot-perp basis" {
  stream spot = binance.spot("XRPUSDT").ticker("1m")
  stream perp = binance.perp("XRPUSDT.P").ticker("1m")

  basis = (perp.price - spot.price) / spot.price * 100

  when basis > 0.50 {
    cex.buy(exchange: "Binance", symbol: "XRPUSDT", order_type: "MARKET", quote: 1000 USDT)
    cex.short(exchange: "Binance Futures", symbol: "XRPUSDT.P", leverage: 1x, quote: 1000 USDT)
  }
...
function createBasisView(code: string, template: StrategyTemplate): EasyViewModel {
  ...
  const nodes: EasyViewNode[] = [
    ...
    },
    {
      id: "spot",
      index: 3,
      title: isFunding ? "BTC 현물 매수" : "XRP 현물 매수",
      subtitle: "CEX spot",
      ...
      params: [
        ...
        { key: "symbol", label: "심볼", value: isFunding ? "BTCUSDT" : "XRPUSDT", helper: "현물 마켓" },
    ...
    },
    {
      id: "perp",
      index: 4,
      title: isFunding ? "BTC 선물 숏" : "XRP 선물 숏",
      subtitle: "CEX perp",
      ...
      params: [
        ...
        { key: "symbol", label: "심볼", value: isFunding ? "BTCUSDT.P" : "XRPUSDT.P", helper: "선물 마켓" },
  ...
  return finalizeEasyViewModel({
    title: isFunding ? "펀딩비 중립 수익 전략" : "XRP 현물-선물 가격차 전략",
...
function inferEasyNodeKind(blockType: string, config: Record<string, unknown>): EasyNodeKind {
  if (config.killSwitch === true || config.emergencyStop === true || config.circuitBreaker === true || isKillSwitchText(Object.values(config).map((value) => String(value ?? "")).join(" "))) {
    return "risk";
  }
...
function buildParamsFromGraphConfig(nodeKind: EasyNodeKind, config: Record<string, unknown>): EasyViewParam[] {
  ...
  const excludeKeys = new Set(["id", "type", "actionType", "name", "label", "title", "description", "summary", "position", "url"]);
...
export function createEasyViewFromStrategyGraph(strategyGraph: StrategyGraphPayload, prompt = ""): EasyViewModel {
  ...
  effectiveConnections.forEach((connection, index) => {
    ...
    if (existing && (edgePriority[normalizeGraphText(existing.kind)] ?? 0) >= (edgePriority[kind] ?? 0)) return;
  ...
  const edges: EasyViewEdge[] = remappedConnections
    .map((connection, index): EasyViewEdge | null => {
      ...
      return {
        ...
        source,
        target,
        label: getEasyConnectionLabel(connectionKind),
        kind,
      };
    })
...
function buildFunctionNodeData(id: string, type: string, config: Record<string, unknown>): FunctionNodeData {
  ...
  const outputName = type === "trigger" ? "signal" : "value";
  ...
  return {
    ...
    inputBlocks: readInputBlocks(config, ["source"]),
    outputBlocks: readOutputBlocks(config, [outputName]),
    inputDescription: readConfigText(config, ["inputDescription"], "연결된 스트림 또는 이전 블록의 값을 사용합니다."),
    logicDescription: readConfigText(config, ["logicDescription", "description"], `${label} 로직을 계산합니다.`),
...
function buildAdvancedInlineTriggerMap(blocks: StrategyGraphBlock[], connections: StrategyGraphConnection[]) {
  ...
  blocks.forEach((block, index) => {
    ...
    const triggerId = getBlockId(block, index);
    const normalSources = getTriggerNormalSources(triggerId, block, blocksById, connections);
    const outgoingActions = getOutgoingConnections(connections, triggerId).filter((connection) => {
      const targetBlock = blocksById.get(normalizeGraphText(connection.toId));
      return normalizeGraphText(connection.kind) === "trigger-action" && getBlockType(targetBlock ?? {}) === "action";
    });

    if (normalSources.length !== 1 || outgoingActions.length === 0) return;
    const sourceId = normalSources[0];
    if (sourceToTrigger.has(sourceId)) return;
...
function buildAdvancedGraphFromStrategyGraph(strategyGraph: StrategyGraphPayload, defaultTitle: string): AdvancedGraph {
  const allBlocks = Array.isArray(strategyGraph.blocks) ? strategyGraph.blocks : [];
  const connections = Array.isArray(strategyGraph.connections) ? strategyGraph.connections : [];
  const allBlocksById = new Map(allBlocks.map((block, index) => [getBlockId(block, index), block]));
  const inlineTriggers = buildAdvancedInlineTriggerMap(allBlocks, connections);
  const hiddenValueBlockIds = new Set(
    allBlocks
      .map((block, index) => ({ block, id: getBlockId(block, index) }))
      .filter(({ block }) => isFixedValueBlock(block))
      .map(({ id }) => id),
  );
  ...
  nodes.push({
    ...
    data: {
      label: defaultTitle || "AI 지능형 파이프라인 (Advanced)",
      styleType: "solid"
    }
  });
  ...
  blocks.forEach((block, index) => {
    ...
    nodes.push({
      ...
      extent: "parent",
      position: { x: 48 + item.level * 340, y: 76 + item.row * 210 },
      data: {
        ...config,
  ...
  visibleConnections.forEach((conn, index) => {
    ...
    const sourceHandle = kind === "trigger-action" && sourceNode.type === "functionNode"
      ? `${sourceNode.id}-block-trigger-out`
      : getOutputHandle(sourceNode);
    edges.push({
...
function resizeAdvancedGroup(nodes: Node[], groupId: string) {
  ...
  const maxRight = children.reduce((max, node) => Math.max(max, node.position.x + 360), 0);
  const maxBottom = children.reduce((max, node) => Math.max(max, node.position.y + 240), 0);
  groupNode.style = {
    ...(groupNode.style ?? {}),
...
function validateAdvancedGraph(graph: AdvancedGraph) {
  ...
  if (!nonGroupNodes.some((node) => node.type === "actionNode")) errors.push("advanced graph has no executable action node");
