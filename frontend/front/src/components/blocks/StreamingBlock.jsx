    ...
    <div className="streaming-block">
      ...
      {mode === 'periodic' && (
        <div className="streaming-block-interval">
          ...
          <NumberStepper
            className="streaming-block-stepper"
            inputClassName="streaming-block-input"
            value={String(interval)}
            min={1}
            step={1}
            onChange={(nextValue) => {
              setInterval(nextValue);
              onUpdateBlock?.(blockId, { updateInterval: Number(nextValue) });
            }}
            decrementLabel="업데이트 주기 감소"
            incrementLabel="업데이트 주기 증가"
          />
        </div>
      )}
