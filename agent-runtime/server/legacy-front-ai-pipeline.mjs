...
import { resolveRequestUserID } from './server/requestUserId.mjs';
import { createMyDataStore } from './server/myDataStore.mjs';
import { registerAIStrategyApi } from './server/api/aiStrategy.mjs';
import { registerCodexStrategyInboxApi } from './server/api/codexStrategyInbox.mjs';
import { registerConfigApi } from './server/api/config.mjs';
import { registerEvmApi } from './server/api/evm.mjs';
import { registerExchangeConnectionsApi } from './server/api/exchangeConnections.mjs';
import { registerHostApi } from './server/api/host.mjs';
import { registerMarketApi } from './server/api/market.mjs';
import { registerStrategyRuntimeApi } from './server/api/strategyRuntime.mjs';
import { registerStreamSampleApi } from './server/api/streamSample.mjs';
...
const AGENT_RUNTIME_BASE_URL = String(process.env.AGENT_RUNTIME_BASE_URL || process.env.HERSHY_AGENT_RUNTIME_BASE_URL || '').trim().replace(/\/+$/, '');
...
const FRONT_API_MODE = String(process.env.FRONT_API_MODE || '').trim().toLowerCase();
const FRONT_USE_MOCK_API = FRONT_API_MODE === 'mock' || ['1', 'true', 'yes'].includes(String(process.env.FRONT_USE_MOCK_API || '').trim().toLowerCase());

if (FRONT_USE_MOCK_API) {
  const [
    { registerMockConfigApi },
    { registerMockMarketApi },
    { registerMockExchangeApi },
    { registerMockStrategyApi },
    { registerMockEvmApi },
    { registerMockHostApi },
  ] = await Promise.all([
    import('./app/mock-api/config.mjs'),
    import('./app/mock-api/market.mjs'),
    import('./app/mock-api/exchange.mjs'),
    import('./app/mock-api/strategy.mjs'),
    import('./app/mock-api/evm.mjs'),
    import('./app/mock-api/host.mjs'),
  ]);
  app.use('/api', express.json({ limit: '8mb' }));
  registerMockConfigApi(app);
  registerMockMarketApi(app);
  registerMockExchangeApi(app);
  registerMockStrategyApi(app);
  registerMockEvmApi(app);
  registerMockHostApi(app);
  console.warn('[front] FRONT_API_MODE=mock: using feature-scoped app/mock-api dummy endpoints before server routes.');
}

registerConfigApi(app, { HOST_API_BASE, FRONT_PORT });
registerMarketApi(app, {
...
registerStrategyRuntimeApi(app, {
  express,
  normalizeObject,
  parseJSON,
  sendError,
...
registerCodexStrategyInboxApi(app, {
  CODEX_STRATEGY_INBOX_PATH,
  crypto,
  express,
  fs,
  fsSync,
...
registerExchangeConnectionsApi(app, {
  buildBalanceMyDataForAI,
  express,
  loadExchangeConnections,
  normalizeText,
  ...
  serializeExchangeConnection,
  serializeExchangeConnections,
  summarizeBinanceAccount,
  upsertBalanceSnapshot,
  upsertExchangeConnection,
});
registerAIStrategyApi(app, {
  agentRuntimeBaseURL: AGENT_RUNTIME_BASE_URL,
  express,
  sendError,
});
registerStreamSampleApi(app, {
  buildPreviewValues,
  derivePayloadFields,
  express,
...
registerEvmApi(app, {
  ...
  isValidEVMAddress,
  normalizeText,
  resolveExplorerAPIKeyFromAuthContext,
  sendError,
});
registerHostApi(app, {
  HOP_BY_HOP_HEADERS,
...
function forceTimeTriggersIntoTriggerBlocks(blocks, connections) {
  ...
  const nextBlocks = blocks.map((block) => {
    ...
    changed = true;
    const restConfig = { ...(block.config || {}) };
    delete restConfig.condition;
    delete restConfig.expression;
    delete restConfig.logic;
    return {
      ...block,
      config: {
        ...restConfig,
